...
This Java program demonstrates various number system conversions using custom methods. The conversions included are:
...
The `Normal` class contains static methods for each conversion. The logic is implemented using basic arithmetic operations and `StringBuilder` for efficient string manipulation.
...
Converts a decimal number to its hexadecimal representation using characters A–F for values 10–15.
...
Converts a binary string (e.g., "1010") to its decimal form.
...
Converts an octal number string (e.g., "42") to its decimal equivalent.
...
1. Save the code in a file named `Normal.java`
2. Compile using:  
   ```
   javac Normal.java
   ```
